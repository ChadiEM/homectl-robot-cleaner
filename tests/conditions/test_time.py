...
@patch('cleaner.conditions.time.Time.now', Mock(return_value=time(9, 45, 5)))
def test_should_fail_condition_in_the_morning():
    assert not Time().is_satisfied()


@patch('cleaner.conditions.time.Time.now', Mock(return_value=time(11, 15, 26)))
def test_should_pass_condition_during_the_day():
    assert Time().is_satisfied()


@patch('cleaner.conditions.time.Time.now', Mock(return_value=time(22, 15, 26)))
